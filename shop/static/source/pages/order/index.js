...
var select_type_delivery = $(".select_type_delivery").select2({
...
select_type_delivery.on("select2:select", function (e) {
  var item = e.params.data.element.dataset.parents;
  var parentsElement = JSON.parse(item);
  var ellementsHidden = [".delivery_address", ".select_delivery_sity", ".select_delivery_department"];
  ellementsHidden.map(function (item) {
    $(item).parents(".input").addClass("hidden");
  });
  parentsElement.map(function (item) {
    $(item).parents(".input").removeClass("hidden");
  });
});
var select_delivery_sity = $(".select_delivery_sity").select2({
  dropdownAutoWidth: true,
  ...
  ajax: {
    ...
    processResults: function processResults(data, params) {
      ...
      var options = data.results.map(function (item) {
        ...
        return {
          id: item.id,
          text: "".concat(item.title, " (").concat(item.region.title, ")"),
          item: item
        };
      });
      return {
...
select_delivery_sity.on("select2:select", function (e) {
  ...
  var item = e.params.data.item.title;
  fetch("https://eleek.starway.agency/api/warehouses/?query=".concat(item), {
    method: 'GET'
  }).then(function (data) {
    return data.json();
  }).then(function (body) {
...
var select_delivery_department = $(".select_delivery_department").select2({
  dropdownAutoWidth: true,
...
var select_type_payment = $(".select_type_payment").select2({
  dropdownAutoWidth: true,
...
_toConsumableArray($("select")).map(function (item) {
  $(item).on("select2:opening", function (e) {
    ...
    if ($(item).parents(".hidden-op").length > 0) {
      var parentsElements = $(item).data("parents");
      var parentsElementsError = $(item).data("error");
      $(parentsElements).parents(".input").append("<label class=\"error\">".concat(parentsElementsError, "</label>"));
...
/* harmony import */ var _components_pages_order_index__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ../components/pages/order/index */ "../components/pages/order/index.js");
// script interface
