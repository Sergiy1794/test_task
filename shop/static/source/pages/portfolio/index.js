...
function resizeNameCategoresInGalery() {
  _toConsumableArray($(".sliders__items")).map(function (item, index) {
    var all_imagesGalery = $(item).find('[data-fancybox="images-preview"]');

    if (all_imagesGalery.length > 0) {
      _toConsumableArray(all_imagesGalery).map(function (item) {
        $(item).data("fancybox", "images-preview".concat(index));
        item.dataset.fancybox = "images-preview".concat(index);
      });
    }
  });
}
...
function slidersTopPositionControl() {
  var slideHeight = $($(".sliders__our_projects").find(".slick-current")[0]).innerHeight();
  $(".sliders__our_projects_control-next")[0].style.top = "".concat(slideHeight / 2, "px ");
  $(".sliders__our_projects_control-prew")[0].style.top = "".concat(slideHeight / 2, "px ");
}
...
function sliders__our_projects() {
  $(".sliders__our_projects").on("init", function (event, slick) {
    setTimeout(function () {
      ...
      _toConsumableArray($(".sliders__items")).map(function (item) {
  ...
  sliders__our_projects.on("beforeChange", function (event, slick, currentSlide, nextSlide) {
    ...
    $(".descriptions_all")[0].innerHTML = $(activeSlider).find(".descriptions")[0].innerHTML;
    console.log();
...
function onShowHoverBoxActive() {
  _toConsumableArray($(".sliders__items")).map(function (item) {
    ...
    $(item).find(".hover_box")[0].style.top = "".concat(heightItem - 20, "px ");
    $(item).on("click", function () {
      if (!$(this).find(".hover_box").hasClass("active")) {
        $(".hover_box").removeClass("active");
        $(this).find(".hover_box").addClass("active");
        var hover_mobile = $(this).find(".hover_box").find(".hover_box_mobile").height();
...
/* harmony import */ var _components_pages_portfolio_index__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ../components/pages/portfolio/index */ "../components/pages/portfolio/index.js");
// script interface
